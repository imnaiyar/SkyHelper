...
<Callout type="info">
  Bot owners can create API keys with custom rate limits and supply them in the `x-api-key` header. The legacy `API_ALLOWLIST_KEY`
  value still bypasses rate limits entirely for trusted internal services.
</Callout>
