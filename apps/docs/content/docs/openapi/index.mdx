...
<Callout type="info">
  Routes under admin category are intentionally not made public as they can only be used by the bot's owner/admins. If you have
  your own running instances, make sure you have included your discord id in `OWNER` array in `config.ts` file. You can check what
  routes are available to the admins by inspecting the `packages/skyhelper/src/api/controllers/admin.controller.ts`
</Callout>
...
<Callout type="info">
  These ratelimits are not applicable to bot's own services. If you are hosting your own instance, make sure that you have
  properly set `x-api-key` headers to your requests with the key you set for `API_ALLOWLIST_KEY` for the running bot/api instance
</Callout>
