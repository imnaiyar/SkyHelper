...
export class SkyHelper extends Client<true> {
    ...
    public contexts = new Collection<
        string,
        ContextMenuCommand<"MessageContext" | "UserContext">
    >();
    ...
    public async loadSlashCmd(dir: string): Promise<void> {
        ...
        for (const filePath of files) {
            ...
            try {
                ...
                if (this.commands.has(command.data.name))
                    throw new Error("The command already exists");
        ...
        this.logger.log(
            `Loaded ${added} Slash Commands. Failed ${failed}`,
            "SLASH"
        );
    }
    ...
    public async loadContextCmd(dir: string): Promise<void> {
        ...
        for (const filePath of files) {
            ...
            try {
                ...
                if (
                    this.contexts.has(
                        command.data.name + command.data.type.toString()
                    )
                )
                    throw new Error("The command already exists");
                // const vld = cmdValidation(command, file);
        ...
        this.logger.log(
            `Loaded ${added} Context Menu Commands. Failed ${failed}`,
            "CONTEXTS"
        );
    }
    ...
    public async loadButtons(dir: string): Promise<void> {
        ...
        for (const filePath of files) {
            ...
            try {
                ...
                if (this.buttons.has(button.data.name))
                    throw new Error("The command already exists");
    ...
    public async loadPrefix(dir: string): Promise<void> {
        ...
        for (const filePath of files) {
            ...
            try {
                ...
                if (typeof command !== "object") continue;
                if (this.prefix.has(command.data.name))
                    throw new Error("The command already exists");
                this.prefix.set(command.data.name, command);
                command.data?.aliases?.forEach((al: string) =>
                    this.prefix.set(al, command)
                );
        ...
        this.logger.log(
            `Loaded ${added} Prefix Commands. Failed ${failed}`,
            "PREFIX"
        );
    }
                    ...
                    default_member_permissions: cmd.data.userPermissions
                        .reduce(
                            (accumulator: bigint, permission) =>
                                accumulator |
                                PermissionFlagsBits[
                                    permission as unknown as keyof typeof PermissionFlagsBits
                                ],
                            BigInt(0)
                        )
                        .toString()
    ...
    public async getCommand(
        ...
        if (!value)
            throw new Error(
                'Command "name" or "id" must be passed as an argument'
            );
