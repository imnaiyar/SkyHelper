...
export default async (
        ...
        try {
            ...
            if (interaction.replied || interaction.deferred) {
                await interaction.followUp({
                    ...
                    embeds: [
                        errorEmbed(
                            t("common.errors.EMBED_TITLE"),
                            t("common.errors.EMBED_DESCRIPTION")
                        )
                    ],
                    components: [
                        errorBtn(t("common.errors.BUTTON_LABEL"), errorId)
                    ]
                });
                return;
            } else {
    ...
    if (interaction.isAutocomplete()) {
        ...
        ) as unknown as SlashCommand<true>;
    ...
    if (interaction.isContextMenuCommand()) {
        const command = client.contexts.get(
            interaction.commandName + interaction.commandType.toString()
        );
        if (!command) {
            await interaction.reply({
                content: t("common.errors.COMMAND_NOT_FOUND"),
    ...
    if (interaction.isButton()) {
        const button = client.buttons.find(btn =>
            interaction.customId.startsWith(btn.data.name)
        );
        if (!button) return;
    ...
    if (interaction.isModalSubmit()) {
        if (interaction.customId === "errorModal") {
            ...
            const commandUsed =
                interaction.fields.getTextInputValue("commandUsed");
            const whatHappened =
                interaction.fields.getTextInputValue("whatHappened");
                    ...
                    await interaction.followUp({
                        embeds: [
                            buildEmbed(
                                fulltimes.geyser,
                                getTimes(0, t, "geyser"),
                                t("times-embed.GEYSER"),
                                "Clement"
                            ).setImage("https://media.discordapp.net/attachments/867638574571323424/1252998364941914243/Visit_Geyser_Clement.png?ex=66744129&is=6672efa9&hm=8d76d1767aca362d23547b1e3beb2b610f58e4fbec24b12af56fdc745f7074e8&")
                        ],
                        ephemeral: true
                    });
                    break;
                case "grandma":
                    await interaction.followUp({
                        embeds: [
                            buildEmbed(
                                fulltimes.grandma,
                                getTimes(0, t, "grandma"),
                                t("times-embed.GRANDMA"),
                                "Clement"
                            ).setImage("https://media.discordapp.net/attachments/867638574571323424/1252998366288416849/Visit_Grandma_Clement.png?ex=6674412a&is=6672efaa&hm=7228b695ec7008204fede2f3d6b4864a06a7cfa25a14ab4d7572957ee940044c&")
                        ],
                        ephemeral: true
                    });
                    break;
...
async function validateCommand(
    command:
        | SlashCommand
        | ContextMenuCommand<"UserContext" | "MessageContext">,
            ...
            if (
                ...
                await interaction.reply({
                    content: t("common.errors.NO_PERMS_BOT", {
                        PERMISSIONS: parsePerms(
                            command.data.botPermissions as unknown as Permission
                        )
                    }),
                    ephemeral: true
                });
