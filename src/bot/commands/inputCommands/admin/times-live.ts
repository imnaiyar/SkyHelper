...
export default {
  async interactionRun(interaction, t) {
    ...
    try {
      await interaction.editReply(await handleLive(client, "SkyTimes", sub, config, t, channel));
    } catch (err) {
  ...
  async messageRun({ message, client, t, args }) {
    ...
    try {
      ...
      await message.reply(await handleLive(client, "SkyTimes", sub, config, t, channel as TextChannel));
    } catch (err) {
