    ...
    if (opt === 'global') {
      ...
      if (globUser) {
       const top = await getAllUser(globUser, client);
        ...
        interaction.editReply({ content: `Oops! Looks like it's all empty. Be the first one to cement your name on the leaderboard. Try the quiz game by running </${interaction.commandName}:${interaction.commandId}`, ephemeral: true });
      }
    } else if (opt === 'server') {
