...
export default {
        ...
        options: [
            {
                ...
                description_localizations: x(
                    "commands.HELP.options.COMMAND.description"
                ),
    ...
    async execute(interaction, t, client) {
        ...
        if (command) {
            ...
            data.setFooter({
                text: t("commands.HELP.RESPONSES_FOOTER_SINGLE"),
        ...
        pageCommands.forEach(cmd => {
            if (
                ...
                totalCommands.push(
                    `</${cmd.name}:${cmd.id}>  \`${
                        cmd.type === 3
                            ? t("commands.HELP.RESPONSES.MESSAGE_APP_DESC")
                            : t("commands.HELP.RESPONSES.USER_APP_DESC")
                    }\`\n\n`
                );
            } else if (
                cmd.options?.some(
                    op =>
                        op.type === 1 ||
                        op.type === ApplicationCommandOptionType.SubcommandGroup
                )
            ) {
                    ...
                    totalCommands.push(
                        `**</${cmd.name} ${o.name}:${cmd.id}>** ${
                            (o as ApplicationCommandSubCommandData).options
                                ?.length
                                ? `${(
                                      o as ApplicationCommandSubCommandData
                                  ).options
                                      ?.map(m => {
                                          return m.required
                                              ? `\`<${m.name}>\``
        ...
        const updateSlashMenu = async () => {
            ...
            slashEmbed.setDescription(
                totalCommands.slice(startIndex, endIndex).join("")
            );
...
function handleCommand(command: ApplicationCommand): EmbedBuilder {
    ...
    const hasSubcommand = command.options?.some(
