...
module.exports = async (int, value, ephemeral, userChoices) => {
  ...
  .setDescription(`### ${data.emote?.icon || data.stance?.icon || data.call?.icon} [${data.name}](https://sky-children-of-the-light.fandom.com/wiki/${data.name.split(' ').join('_')})`)
  .addFields(
    {
        name: 'Type',
        value: data.type,
  ...
  if (data.season) embed.addFields({ name: 'Season', value: `${int.client.emojisMap.get('season')[data.season]} ${data.season}` })
  if (data.ts) embed.addFields({ name: 'TS Summary', value: data.ts.returned ? data.ts.dates.map(date => `\n- ${date}`).join('\n') : 'This spirit has not returned yet'})
  ...
  const emoteBtn = new ButtonBuilder()
  .setCustomId('spirit_emote')
  .setStyle('1')
  .setLabel('Emote');
  ...
  const cosmeticBtn = new ButtonBuilder()
    .setCustomId('spirit_cosmetic')
    .setStyle('1')
    .setLabel('Cosmetics'); 

  const callBtn = new ButtonBuilder()
  .setCustomId('spirit_call')
  .setLabel('Call')
  .setStyle('1')
  

  const stanceBtn = new ButtonBuilder()
  .setCustomId('spirit_stance')
  .setLabel('Stance')
  .setStyle('1')
...
async function handleEmote(int, data, collector) {
    ...
    .setDescription(`### ${data.emote.icon} [${emote.title}](https://sky-children-of-the-light.fandom.com/wiki/${data.name.split(' ').join('_')}#Expression)`)
    ...
    .addComponents(
        ...
        .setDisabled(page === 1),
        new ButtonBuilder()
        .setCustomId('spirit_emote_home')
        .setEmoji(data.emote.icon)
        .setStyle('3'),
        new ButtonBuilder()
