...
client.on('messageCreate', async (message) => {
  ...
  const chunkSize = 2000;
      ...
      if (requestsRemaining === '0') {
        message.reply(`Daily request limit reached, wait for ~6 minutes for one more request.\n\n**OpenAI error code**: ${errorCode}\n**Remaining requests**: ${requestsRemaining}\n**Time remaining for max requests(200) reset**: ${remainingTime} (This will increase everytime you make a request)`)
              } else if (errorCode === 429 && requestsRemaining !== '0') {
                message.reply(`OpenAI ERROR CODE: ${errorCode}. Too many requests, only 3 requests allowed per minute, kindly wait before making a request.`)
              }
      console.error('OpenAI API error:', error);
