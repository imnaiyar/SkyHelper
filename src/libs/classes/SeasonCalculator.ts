...
import {
    ...
    SeasonData as ssn
} from "#libs/constants/seasonPrices";
...
export default class {
    ...
    public sync(): this {
        ...
        this.totalCurrency = Object.keys(prices)
            .map(k => {
                ...
                return spirit.cosmetics.reduce(
        ...
        const candlesUsed = Object.keys(this.data.spirits)
            .map(k => {
                ...
                return spirit.cosmetics
                    .filter(sp => sp.acquired)
                    .reduce((acc, value) => acc + value.price, 0);
            })
            .reduce((acc, num) => acc + num, 0);
        this.requiredCurrency = this.totalCurrency - candlesUsed;
        this.requiredCurrency =
            this.requiredCurrency < 0 ? 0 : this.requiredCurrency;
        const divider = this.data.hasPass ? 6 : 5;
        this.remainingDays = (this.requiredCurrency - this.candles) / divider;
        const now = moment().tz("America/Los_Angeles");
        const seasonEnds = moment
            .tz(ssn.end, "DD-MM-YYYY", "America/Los_Angeles")
            .endOf("day");
        this.seasonEndsIn = seasonEnds.diff(now, "days");
        this.obtainableCurrency =
            (this.data.hasPass
                ? 6
                : 5 * ((this.dailies ? 0 : 1) + this.seasonEndsIn)) +
            this.candles;
        this.remainingCandles = this.obtainableCurrency - this.requiredCurrency;
        this.remainingCandles =
            this.remainingCandles < 0 ? 0 : this.remainingCandles;

        this.progressLevel =
            100 - (this.requiredCurrency / this.totalCurrency) * 100;
        return this;
    }
    ...
    private getAcquiredItems() {
        ...
        for (const spirit of Object.keys(spirits)) {
            ...
            SpiritsProgress.push({
                ...
                items: spiritData.cosmetics.map(
                    item =>
                        item.icon +
                        " " +
                        (item.acquired
    ...
    public async getContent(): Promise<InteractionUpdateOptions> {
        ...
        const emoji = client.emojis.cache.get(
            parseEmoji(this.seasonIcon)?.id ?? ""
        );
        const startDate = moment
            .tz(ssn.start, "DD-MM-YYYY", "America/Los_Angeles")
            .startOf("day");
        ...
        const embeds = new EmbedBuilder()
            ...
            .setDescription(
                this.t("commands.SEASONAL_CALCULATOR.RESPONSES.EMBED_DESCRIPTION", {
                    ...
                    POSSIBLE:
                        this.remainingDays > this.seasonEndsIn
                            ? "(Not Possible)"
                            : "",
            ...
            .setTitle(
                `${this.seasonIcon} ${this.season} (${time(
                    startDate.unix(),
                    "D"
                )} - ${time(endDate.unix(), "D")}) ( ${this.t(
                    "commands.SEASONAL_CALCULATOR.RESPONSES.ENDS_IN",
                    { DAYS: this.seasonEndsIn }
                )})`
            )
            .setURL(
                `https://sky-children-of-the-light.fandom.com/wiki/${this.season
                    .split(" ")
                    .join("_")}`
            );
    ...
    async handleInt(interaction: ChatInputCommandInteraction) {
        ...
        collector.on("collect", async int => {
            ...
            this.data.spirits[spirit].cosmetics.forEach(
                it => (it.acquired = false)
            );
            values.forEach(i => {
                this.data.spirits[spirit].cosmetics[parseInt(i)].acquired =
                    true;
            });
    ...
    public async buildCard(): Promise<Buffer> {
        ...
        const card = new SeasonProgressCard()
            .setName(
                this.author instanceof GuildMember
                    ? this.author.nickname ?? this.author.displayName
