...
const lctnBtn = new ButtonBuilder()
    .setCustomId("spirit_location")
    .setLabel("Location")
    .setStyle(ButtonStyle.Secondary);

// Define Friendship Tree button
const treeBtn = new ButtonBuilder()
    .setCustomId("spirit_tree")
    .setStyle(ButtonStyle.Secondary)
...
export class Spirits {
    ...
    public getEmbed(): EmbedBuilder {
        ...
        const desc = `**Type:** ${data.type}${
            data.realm
                ? `\n**Realm:** ${
                      client.emojisMap.get("realms")![data.realm]
                  } ${data.realm}`
                : ""
        }${
            data.season
                ? `\n**Season:** ${
                  ...
                  } Season of ${data.season}`
                : ""
        }`;
        const embed = new EmbedBuilder()
            .setTitle(`${icon} ${data.name}`)
            .setURL(
                `https://sky-children-of-the-light.fandom.com/wiki/${data.name
                    .split(" ")
                    .join("_")}`
            )
            .setDescription(desc)
                    ...
                    ? `Total Visits: ${
                          ...
                          .map(date => {
                              ...
                              const dateE = dateM
                                  ...
                                  .add(3, "days")
                                  .endOf("day");
                              return `- ${time(dateM.toDate(), "D")} - ${time(
                                  dateE.toDate(),
                                  "D"
                              )} ${index}`;
                          })
        ...
        if (data.main) {
            ...
            embed.addFields({
                name: `${
                    data.ts?.returned
                        ? "Friendship Tree (Last Visit)"
                        : "Seasonal Price Chart"
                } by ${data.tree!.by}`,
                value: data
                    .tree!.total.replaceAll(
                        ":RegularCandle:",
                        "<:RegularCandle:1207793250895794226>"
                    )
                    .replaceAll(
                        ":RegularHeart:",
                        "<:regularHeart:1207793247792013474>"
                    )
     ...
     */
    public async handleInt(
        interaction: ChatInputCommandInteraction
    ): Promise<InteractionCollector<ButtonInteraction> | undefined> {
        const data = this.data;
        ...
        try {
            ...
            collector.on(
                "collect",
                async (int: ButtonInteraction): Promise<void> => {
                    if (int.user.id !== interaction.user.id) {
                        await int.reply({
                            content: "You cannot use menu(s) meant for others!",
                            ephemeral: true
                        });
                        return;
                    }
                    ...
                    switch (int.customId) {
                        ...
                        case "spirit_location": {
                            ...
                            newEmbed.setImage(
                                `${config.CDN_URL}/${data.location!.image}`
                            );
                            await int.editReply({
                                embeds: [newEmbed],
                                ...(index === 1
                        ...
                        case "spirit_tree": {
                            ...
                            newEmbed.spliceFields(-1, 1, {
                                ...
                                value: data
                                    ...
                                    .replaceAll(
                                        ":AC:",
                                        "<:AscendedCandle:1207793254301433926>"
                                    )
                            });
                            newEmbed.setImage(
                                `${config.CDN_URL}/${data.tree!.image}`
                            );
                            await int.editReply({
                                embeds: [newEmbed],
                                ...(index === 1
                        ...
                        case "spirit_call": {
                            await int.editReply({
                                ...
                                embeds: [],
                                files: [
                                    `${config.CDN_URL}/${data.call!.image}`
                                ],
                                // prettier-ignore
                        ...
                        case "spirit_stance": {
                            const stanceEmbed = new EmbedBuilder()
                                .setTitle(
                                    `${data.stance!.icon} ${data.stance!.title}`
                                )
                                .setURL(
                                    `https://sky-children-of-the-light.fandom.com/wiki/${data.name
                                        .split(" ")
                                ...
                                .setDescription(
                                    `Stance preview (Standing, sitting, kneeling and laying).`
                                )
                                .setImage(
                                    `${config.CDN_URL}/${data.stance!.image}`
                                )
            ...
            collector.on("end", async (_coll, reason) => {
                ...
                const components = msg.components.map(row => {
                    ...
                    actionRow.components.forEach(com =>
                        (com as unknown as any).setDisabled(true)
                    );
                    return actionRow;
     ...
     */
    private async handleExpression(
        data: SpiritsData,
        ...
        const getEmote = (): {
            ...
            const embed = new EmbedBuilder()
                .setAuthor({
                    name: `${data.emote ? "Emote" : "Friend Action"} - ${
                        data.name
                    }`
                })
            ...
            const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
                new ButtonBuilder()
                    ...
                    .setLabel(
                        `⬅️ ${
                            exprsn!.level[page - 2]?.title.slice(-7) ||
                            emote.title.slice(-7)
                        }`
                    )
                    .setStyle(ButtonStyle.Secondary)
                ...
                new ButtonBuilder()
                    ...
                    .setLabel(
                        `${
                            exprsn!.level[page]?.title.slice(-7) ||
                            emote.title.slice(-7)
                        } ➡️`
                    )
                    .setStyle(ButtonStyle.Secondary)
        ...
        try {
            ...
            const collector = btnInt.message.createMessageComponentCollector({
                filter: i =>
                    ["emote_prev", "emote_next", "emote_back"].includes(
                        i.customId
                    ),
                idle: 60_000
            });
