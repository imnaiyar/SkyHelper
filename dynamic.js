import { generateSvg, registerFont } from "./svg.js";
import { createCanvas, loadImage } from "@napi-rs/canvas";
import { cropImage } from "cropify";
const Dynamic = async (option) => {
  if (!option.progress) option.progress = 1;
  if (!option.name) option.name = "NyR";
  if (!option.author) option.author = "Season of Nesting";
  if (!option.seasonIcon)
    option.seasonIcon =
  ...
  if (!option.thumbnailImage) {
    option.thumbnailImage = "https://cdn.discordapp.com/avatars/851588007697580033/9e96b03eb19bd3913268dcaab417578b.png";
  }
  ...
  try {
    ...
    ctx.fillText(`${option.progress === 99.999 ? 100 : option.progress}%`, 2040, 280);
    // Season Title
    ...
    ctx.fillText(option.author, 650, 350);
