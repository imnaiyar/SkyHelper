...
export class ProfileDisplay extends BasePlannerHandler {
  ...
  async profiledisplay() {
    const currencies = formatCurrencies(this.data, this.settings.plannerData ?? this.planner.PlannerDataHelper.createEmpty());
    const unlocked = formatUnlockedItems(this.data);
    ...
    const profilecard = await generatePlannerProfileCard({
      ...
      progress: calculateUserProgress(this.data),
    });
    return {
      components: [
        container(
  ...
  currencydisplay() {
    const breakdown = this.planner.calculateCurrencyBreakdown(this.data);

    // Check if there's any spent currency data
    const hasSpentData = !isEmptyCost(breakdown.total.cost) || breakdown.total.price > 0;
      ...
      components: [
        container(
          ...
          textDisplay(
            ...
            !this.planner.isEmptyCost(breakdown.total.cost) || breakdown.total.price > 0
              ? `# Total Spent:\n${formatBreakdownCost(breakdown.total)}`
              : "",
            !this.planner.isEmptyCost(breakdown.regular.cost) || breakdown.regular.price > 0
  ...
  private seasonsSpent(breakdown: IBreakdownData) {
    ...
    const sortedSeasons = Array.from(breakdown.seasons.entries())
      .map(([guid, cost]) => ({
        season: this.data.seasons.find((s) => s.guid === guid),
        cost,
      }))
      .filter((s) => s.season)
  ...
  private eventsSpent(breakdown: IBreakdownData) {
    ...
    const sortedEvents = Array.from(breakdown.events.entries())
      .map(([guid, cost]) => ({
        event: this.data.events.find((e) => e.guid === guid),
        cost,
      }))
      .filter((e) => e.event)
      .sort((a, b) => {
    ...
    for (const { event, cost } of sortedEvents) {
      if (!event || (this.planner.isEmptyCost(cost.cost) && cost.price === 0)) continue;
