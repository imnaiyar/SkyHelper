...
import { FilterManager, FilterType, type CustomFilterConfigs } from "./filter.manager.js";
...
export class BasePlannerHandler {
  ...
  constructor(
    ...
    if (this.state.filter) {
      this.filterManager = new FilterManager(this.data, this.state.filter, [], this.customFilterConfigs);
    }
  }
  ...
  protected initializeFilters(supportedFilters: FilterType[], customConfigs?: CustomFilterConfigs): void {
    ...
    this.filterManager = new FilterManager(this.data, this.state.filter, supportedFilters, customConfigs);
    ...
    if (total > 1) {
      components.push(
        this.paginationBtns({
          ...
          total,
          tab: this.state.tab,
          filter: this.filterManager?.serializeFilters(),
  ...
  createCustomId(opt: Partial<NavigationState>) {
    ...
    const {
      ...
      filter = redirect(this.filterManager?.serializeFilters()),
  ...
  protected createFilterIndicator(): string | null {
    ...
    for (const filterType of this.supportedFilters) {
      ...
      if (values.length > 0) {
        activeFilters.push(`${type ?? "Unknown Filter"}: \`${values.join("|")}\``);
      }
    }
