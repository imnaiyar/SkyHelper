...
const configs = (planner: BasePlannerHandler): Array<{ title: string; description: string; custom_id: string }> => [
  {
    ...
    custom_id: planner.createCustomId({
      ...
      f: serializeFilters(
        new Map([
          [FilterType.Shops, planner.data.shops.filter((s) => s.permanent === "event").map((s) => s.guid)],
  ...
  },
  {
    ...
    custom_id: planner.createCustomId({
      ...
      f: serializeFilters(
        new Map([
          [FilterType.Shops, planner.data.shops.filter((s) => s.permanent === "harmony").map((s) => s.guid)],
  ...
  },
  {
    ...
    custom_id: planner.createCustomId({
      d: "shops",
      f: serializeFilters(
        new Map([[FilterType.Shops, planner.data.shops.filter((s) => s.permanent === "office").map((s) => s.guid)]]),
      ),
    }),
  },
];
export class ShopHomeDisplay extends BasePlannerHandler {
  ...
  shops: IShop[] = [];
  loadData() {
    this.shops = this.data.shops.filter((s) => s.permanent === true);
