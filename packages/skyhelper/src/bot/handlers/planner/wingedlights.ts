...
import { ItemType, type ISkyData, type IWingedLight } from "skygame-data";
...
export class WingedLightsDisplay extends BasePlannerHandler {
  ...
  override handle() {
    const wls = this.filterWls(this.data.wingedLights.items);
    return {
      components: [
        container(
          this.getTopBtns(),
  ...
  private verifyRefWl(wl: IWingedLight, data?: string) {
  ...
  public filterWls(wls: IWingedLight[]) {
    let winged = [...wls];
  ...
  private getTopBtns() {
    const remainingWls = this.data.wingedLights.items.filter((wl) => !wl.unlocked);
    return row(
      [
        ...
        this.viewbtn(createActionId({ action: PlannerAction.ToggleWL, actionType: "all", navState: this.state }), {
          label: "All Found" + ` (${remainingWls.length}/${this.data.wingedLights.items.length})`,
        ...
        this.viewbtn(createActionId({ action: PlannerAction.ToggleWL, actionType: "reset", navState: this.state }), {
          label: "Reset All",
          style: 4,
          disabled: remainingWls.length === this.data.wingedLights.items.length,
        }),
