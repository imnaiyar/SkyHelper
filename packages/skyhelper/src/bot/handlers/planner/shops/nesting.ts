...
import { nestingconfigs, type INestingConfig, type IRotation, type IRotationItem } from "./config.js";
...
export class NestingWorkshopDisplay extends BasePlannerHandler {
  ...
  private challengeSpirits: ISpirit[] = [];
  ...
  private initializeNestingData(): void {
    ...
    // items list
    const listnodes = this.data.itemLists.items.find((l) => l.guid === "AKNI67tVW-");
    if (listnodes) {
    ...
    for (const [index, rotation] of nestingconfigs.rotations.entries()) {
      for (const item of rotation.items) {
        const gameItem = this.data.items.items.find((i) => i.guid === item.guid);
        if (gameItem) {
          item.item = gameItem;
        }
    ...
    for (const item of this.permanentItems) {
      const gameItem = this.data.items.items.find((i: any) => i.guid === item.guid);
      if (gameItem) {
        item.item = gameItem;
      }
    ...
    this.challengeSpirits = nestingconfigs.challengeSpirits
      .map((guid: string) => this.data.spirits.items.find((s: any) => s.guid === guid))
      .filter(Boolean) as ISpirit[];
            ...
            itemCallback: (a) => [
              section(
                ...
                a.items
                  .map(
                    (i) =>
                      `${this.formatemoji(i.item!.emoji, i.item!.name)} ${i.item!.name}${"quantity" in i ? ` (X${i.quantity ?? 1})` : ""} - ${CostUtils.costToEmoji(i) ?? "Free"}`,
                  )
                  .join("\n"),
              ),
            ],
  ...
  private createItemTitle(item: IRotationItem | IItemListNode, showCosts = false) {
    ...
    const infos = [
      ...
      showCosts ? (CostUtils.costToEmoji(item) ?? "Free") : "",
      item.item?.unlocked ? "**Unlocked**" : "",
      this.nestingRotationData?.[item.guid]
        ? `**Inventory**: ${this.nestingRotationData[item.guid]?.q ?? 0} (Cost: ${CostUtils.costToEmoji(this.nestingRotationData[item.guid]!.cost) ?? "Free"})`
        : "",
    ];
    return infos.join("\n");
  }
