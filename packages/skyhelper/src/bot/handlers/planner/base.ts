import { emojis, realms_emojis } from "@skyhelperbot/constants";
import {
  type _Nullable,
...
export abstract class BasePlannerHandler {
  ...
  createTopCategoryRow(selected: DisplayTabs, user?: string, back?: { page?: number }) {
    ...
    const seasonIcon = PlannerService.getCurrentSeason(this.data)?.emoji ?? this.data.seasons.items[0]?.emoji;
        ...
        .map(([label, category]) => {
          ...
          const icon =
            category === DisplayTabs.Seasons
              ? (PlannerService.getCurrentSeason(this.data)?.emoji ?? this.data.seasons.items[0]?.emoji)
  ...
  formatDateTimestamp(date: DateTime, style?: string) {
    return `<t:${Math.floor(date.toMillis() / 1000)}${style ? `:${style}` : ""}>`;
  }
  ...
  private formatFilterValues(filterType: FilterType, values: string[]) {
    switch (filterType) {
      case FilterType.Realms: {
        const formatted = values.map((v) => this.data.realms.items.find((r) => r.guid === v)?.shortName ?? v);
        return formatted;
      }
      case FilterType.Events: {
        const formatted = values.map((v) => {
          const event = this.data.events.items.find((r) => r.guid === v);
      ...
      case FilterType.Seasons: {
        const formatted = values.map((v) => {
          const season = this.data.seasons.items.find((r) => r.guid === v);
      ...
      case FilterType.Areas: {
        const formatted = values.map((v) => this.data.areas.items.find((a) => a.guid === v)?.name ?? v);
        return formatted;
      }
      default:
        return values;
