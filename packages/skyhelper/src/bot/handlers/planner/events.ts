...
export class EventsDisplay extends BasePlannerHandler {
  ...
  override async handle() {
    ...
    if (this.state.it) {
      const event = this.data.events.items.find((e) => e.guid === this.state.it);
      ...
      const events = order ? this.sortEvents(this.data.events.items, order) : this.data.events.items;
      components.push(
        container(
    ...
    return this.displayPaginatedList({
      ...
      itemCallback: (event) => [
        section(
          ...
          `**${event.name}**`,
          [event.instances?.map((i) => i.date.year.toString()), event.recurring ? "Recurring" : ""]
            .filter((s) => !!s)
            .flat()
            .join(" • "),
  ...
  private sortEvents(evs: IEvent[], order: string) {
