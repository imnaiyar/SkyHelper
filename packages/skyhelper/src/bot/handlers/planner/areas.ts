...
import { type IArea } from "skygame-data";
...
export class AreasDisplay extends BasePlannerHandler {
  ...
  override handle() {
    const area = this.state.it ? this.data.areas.items.find((a) => a.guid === this.state.it) : null;
    if (area) {
      return { components: [container(this.areadisplay(area))] };
    }
    const areas = this.filterAreas(this.data.areas.items);
    return {
      components: [
        container(
