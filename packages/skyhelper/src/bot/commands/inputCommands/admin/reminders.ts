...
async function handleSetup(
  ...
  const initialEmbed = async () => {
    return new EmbedBuilder()
      ...
      .addFields(
        { name: `**•** ${t("features:times-embed.GEYSER")} ${getActive(geyser)}`, value: "\u200B", inline: true },
        { name: `**•** ${t("features:times-embed.GRANDMA")} ${getActive(grandma)}`, value: "\u200B", inline: true },
        { name: `**•** ${t("features:times-embed.TURTLE")} ${getActive(turtle)}`, value: "\u200B", inline: true },
        { name: `**•** ${t("features:times-embed.DAILY-RESET")} ${getActive(reset)}`, value: "\u200B", inline: true },
        { name: `**•** ${t("features:times-embed.EDEN")} ${getActive(eden)}`, value: "\u200B", inline: true },
        { name: `**•** ${t("features:reminders.DAILY_QUESTS")} ${getActive(dailies)}`, value: "\u200B", inline: true },
      )
      .setDescription(
  ...
  collector.on("collect", async (int) => {
    ...
    if (int.user.id !== user.id) {
      ...
      await int.editReply(ts("errors:NOT-ALLOWED"));
      return;
    }
    try {
      await int.deferReply({ ephemeral: true });
