...
export default {
  interactionRun: async ({ helper, options }) => {
    ...
    switch (sub) {
      case "info": {
        await handleInfo(helper, helper.client.utils.createdTimeStamp(helper.int.id));
        break;
      }
      case "manage": {
      ...
      default:
        await helper.reply({ content: "Not implemented yet. Sorry!", flags: 64 });
    }
  },
  ...BOT_COMMAND_DATA,
} satisfies Command;

async function handleInfo(helper: InteractionHelper, time: number) {
  await helper.defer();
