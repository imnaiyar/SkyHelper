...
export default {
  ...
  async interactionRun({ helper, options }) {
    ...
    try {
      ...
      if (subcommand === "stats") {
        ...
        if (events.current.length > 0) {
          ...
          for (const eventInfo of events.current) {
            const startDate = new Date(eventInfo.instance.date).toLocaleDateString();
            const endDate = new Date(eventInfo.instance.endDate).toLocaleDateString();
      ...
      console.error("SkyGame Planner command error:", error);
      await helper.editReply({
        components: [
          textDisplay("# Error"),
          separator(true, 1),
          textDisplay("There was an error fetching SkyGame Planner data. Please try again later."),
        ],
        flags: MessageFlags.IsComponentsV2,
      });
    }
