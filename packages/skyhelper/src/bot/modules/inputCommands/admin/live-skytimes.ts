...
import { TIMES_LIVE_DATA } from "@/modules/commands-data/admin-commands";
...
export default {
  async interactionRun({ interaction, t, helper, options }) {
    ...
    try {
      await helper.editReply(await handleLive(client, "SkyTimes", sub, config, t, channel));
    } catch (err) {
      client.logger.error("Failed to stop skytimes Updates in " + guild.name, err);
    }
  },
  ...TIMES_LIVE_DATA,
} satisfies Command;
