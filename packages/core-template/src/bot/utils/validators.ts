...
import type { MessageFlags } from "./MessageFlags.js";
import { PermissionsUtil } from "./PermissionUtils.js";
 ...
 */
export function validateInteractions({ command, interaction, options, client, t }: ValidateInteractionOptions):
  | {
      status: false;
      message: string;
    }
  ...
  const user = interaction.member?.user ?? interaction.user!; // invoking user
  ...
  if (interaction.guild_id && command.botPermissions) {
    ...
    if (toCheck) {
      ...
      const botPerms = interaction.app_permissions as `${number}`;

      if (toCheck && !client.permUtils(botPerms).has(command.botPermissions)) {
        const missingPerms = client.permUtils(botPerms).missing(command.botPermissions);
        return {
          status: false,
  ...
  if (command.validations) {
    for (const validation of command.validations) {
      if (validation.type !== "message") {
        ...
        const validated = await validation.callback(interaction, t);
        ...
        embeds: [
          {
            ...
            author: {
              ...
              icon_url: client.getUserAvatar(client.user),
            },
